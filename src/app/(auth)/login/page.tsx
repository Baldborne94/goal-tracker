...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (result?.error) { setError("Email o password errati"); setLoading(false); }
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-[#16112e] rounded-2xl border border-[#3b2d6e] p-6 space-y-4 shadow-xl shadow-purple-950/50">
          ...
          <div>
            ...
            <input type="password" required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-[#0f0d22] border border-[#3b2d6e] focus:outline-none focus:ring-2 focus:ring-amber-500/50 text-[#ede9ff] placeholder-[#4a3a7a]" placeholder="••••••••" />
          </div>
        ...
        <p className="text-center text-sm text-[#6b5a9e] mt-4">Non hai un account?{" "}<Link href="/register" className="text-amber-400 font-medium hover:text-amber-300">Inizia l&apos;avventura</Link></p>
      </div>
    </div>
  );
}
