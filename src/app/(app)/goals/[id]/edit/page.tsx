...
export default async function EditGoalPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [goal, categories] = await Promise.all([
    prisma.goal.findFirst({ where: { id, userId: session!.user!.id! }, include: { tags: { include: { tag: true } }, milestones: { orderBy: { order: "asc" } } } }),
    prisma.category.findMany({ orderBy: { name: "asc" } }),
  ]);
  if (!goal) notFound();
  const initialData = {
    id: goal.id, title: goal.title, description: goal.description || "", priority: goal.priority,
