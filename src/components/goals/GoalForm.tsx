...
type Props = {
  ...
  initialData?: {
    ...
    targetDate: string; categoryId: string; tags: string[]; milestones: string[];
  };
};
...
export default function GoalForm({ categories, initialData }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    priority: initialData?.priority || "medium", targetDate: initialData?.targetDate || "",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <input required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Cosa vuoi raggiungere?" className={inputClass} />
      </div>

      <div>
        <label className="block text-sm font-medium text-[#c4b5fd] mb-1">Descrizione</label>
        <textarea rows={3} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Descrivi la tua missione..." className={`${inputClass} resize-none`} />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => setForm({ ...form, categoryId: "" })} className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors border ${
          ...
          {categories.map((c) => (
            <button key={c.id} type="button" onClick={() => setForm({ ...form, categoryId: c.id })}
              ...
              style={form.categoryId === c.id ? { backgroundColor: c.color, borderColor: c.color } : { borderColor: "#3b2d6e" }}>
              {c.name}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex gap-2 mb-2">
          <input value={tagInput} onChange={(e) => setTagInput(e.target.value)}
            ...
            placeholder="Aggiungi tag..." className="flex-1 px-4 py-2.5 rounded-xl bg-[#0f0d22] border border-[#3b2d6e] focus:outline-none focus:ring-2 focus:ring-amber-500/40 text-[#ede9ff] placeholder-[#4a3a7a] text-sm" />
          ...
          <div className="flex flex-wrap gap-2">
            {tags.map((t) => (
              <span key={t} className="flex items-center gap-1 text-xs bg-[#1e1740] text-[#9d8ac7] px-3 py-1 rounded-full border border-[#3b2d6e]">
                #{t}<button type="button" onClick={() => setTags(tags.filter((x) => x !== t))} className="ml-0.5 text-[#6b5a9e] hover:text-red-400">×</button>
              </span>
            ))}
          </div>
        )}
        ...
        <div>
          ...
          <div className="flex gap-2 mb-2">
            <input value={milestoneInput} onChange={(e) => setMilestoneInput(e.target.value)}
              ...
              placeholder="Aggiungi una tappa..." className="flex-1 px-4 py-2.5 rounded-xl bg-[#0f0d22] border border-[#3b2d6e] focus:outline-none focus:ring-2 focus:ring-amber-500/40 text-[#ede9ff] placeholder-[#4a3a7a] text-sm" />
              ...
              {milestones.map((m, i) => (
                <div key={i} className="flex items-center justify-between bg-[#0f0d22] border border-[#2a1f50] rounded-xl px-4 py-2.5 text-sm text-[#ede9ff]">
                  <span className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-amber-900/40 text-amber-400 text-xs flex items-center justify-center font-semibold border border-amber-700/40">{i + 1}</span>
                    {m}
                  </span>
