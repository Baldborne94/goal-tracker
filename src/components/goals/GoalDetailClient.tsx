...
type Goal = {
  ...
  progress: number; targetDate: string | null; completedAt: string | null; points: number;
  category: Category; milestones: Milestone[]; tags: GoalTag[];
};
...
export default function GoalDetailClient({ goal: initial, priorityLabel, priorityColor, formattedDate }: Props) {
  ...
  async function toggleMilestone(milestoneId: string, completed: boolean) {
    ...
    if (res.ok) {
      ...
      setGoal((prev) => ({ ...prev, progress, status: progress === 100 ? "completed" : "active",
    ...
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      <div className="bg-[#16112e] rounded-2xl border border-[#3b2d6e] p-5 mb-4">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {goal.category && (
            <span className="text-xs px-2.5 py-1 rounded-full font-medium" style={{ backgroundColor: goal.category.color + "25", color: goal.category.color }}>{goal.category.name}</span>
          )}
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${priorityColor}`}>{priorityLabel}</span>
          {goal.tags.map(({ tag }) => (
            <span key={tag.id} className="text-xs px-2.5 py-1 rounded-full bg-[#1e1740] text-[#9d8ac7] border border-[#3b2d6e]">#{tag.name}</span>
          ))}
        </div>
          ...
          <div className="space-y-3">
            {goal.milestones.map((m) => (
              <div key={m.id}
                ...
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-colors border-2 ${
