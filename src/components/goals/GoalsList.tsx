    ...
    <>
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {(["all", "active", "completed"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${
            filter === f ? "bg-amber-500 text-black border-amber-500 font-bold" : "bg-[#16112e] text-[#9d8ac7] border-[#3b2d6e]"
          }`}>{f === "all" ? "Tutte" : f === "active" ? "⚡ Attive" : "👑 Completate"}</button>
        ))}
      </div>
      {categories.length > 0 && (
        <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
          <button onClick={() => setCategoryId("all")} className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium border ${
            categoryId === "all" ? "bg-[#ede9ff] text-[#0c0a1a] border-[#ede9ff]" : "bg-[#16112e] text-[#9d8ac7] border-[#3b2d6e]"
          }`}>Tutte</button>
          {categories.map((c) => (
            <button key={c.id} onClick={() => setCategoryId(c.id)} className={`flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium border ${
              categoryId === c.id ? "text-black" : "bg-[#16112e] text-[#9d8ac7]"
            }`} style={categoryId === c.id ? { backgroundColor: c.color, borderColor: c.color } : { borderColor: "#3b2d6e" }}>{c.name}</button>
          ))}
        </div>
      )}
      {filtered.length === 0 ? (
        <div className="bg-[#16112e] rounded-2xl border border-[#3b2d6e] p-8 text-center">
          ...
          <Link href="/goals/new" className="inline-block mt-4 px-5 py-2.5 bg-gradient-to-r from-amber-500 to-yellow-400 text-black rounded-xl text-sm font-bold">Crea missione</Link>
        </div>
      ) : (
...
function GoalCard({ goal }: { goal: Goal }) {
  ...
  return (
    <Link href={`/goals/${goal.id}`} className="block bg-[#16112e] rounded-2xl border border-[#3b2d6e] p-4 hover:border-amber-500/40 transition-colors">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        }`}>{goal.status === "completed" ? "👑" : "⚡"}{" "}{goal.status === "completed" ? "Fatto" : "Attiva"}</span>
      </div>
      <div className="flex flex-wrap items-center gap-2 mb-3">
        {goal.category && <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: goal.category.color + "25", color: goal.category.color }}>{goal.category.name}</span>}
        ...
        {goal.tags.slice(0, 2).map(({ tag }) => <span key={tag.id} className="text-xs px-2 py-0.5 rounded-full bg-[#1e1740] text-[#9d8ac7] border border-[#3b2d6e]">#{tag.name}</span>)}
      </div>
      <div>
      ...
      {goal.targetDate && <p className="text-xs text-[#6b5a9e] mt-2">🌙 {formatDate(goal.targetDate)}</p>}
    </Link>
  );
}
