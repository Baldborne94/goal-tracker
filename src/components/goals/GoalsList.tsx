        ...
        {(["all", "active", "completed"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`flex-shrink-0 px-4 py-1.5 rounded-full text-sm font-medium transition-colors border ${
              filter === f ? "bg-amber-500 text-black border-amber-500 font-bold" : "bg-[#16112e] text-[#9d8ac7] border-[#3b2d6e] hover:border-amber-500/40"
            }`}>
        ...
        <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
          ...
          {categories.map((c) => (
            <button key={c.id} onClick={() => setCategoryId(c.id)}
              ...
              style={categoryId === c.id ? { backgroundColor: c.color, borderColor: c.color } : { borderColor: "#3b2d6e" }}>
              {c.name}
            </button>
          ))}
        </div>
    ...
    <Link href={`/goals/${goal.id}`} className="block bg-[#16112e] rounded-2xl border border-[#3b2d6e] p-4 hover:border-amber-500/40 transition-colors">
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 font-medium border ${
          ...
          {goal.status === "completed" ? "👑" : "⚡"}{" "}{goal.status === "completed" ? "Fatto" : "Attiva"}
        </span>
      </div>
      <div className="flex flex-wrap items-center gap-2 mb-3">
        {goal.category && (
          <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: goal.category.color + "25", color: goal.category.color }}>{goal.category.name}</span>
        )}
        ...
        {goal.tags.slice(0, 2).map(({ tag }) => (
          <span key={tag.id} className="text-xs px-2 py-0.5 rounded-full bg-[#1e1740] text-[#9d8ac7] border border-[#3b2d6e]">#{tag.name}</span>
        ))}
      </div>
      <div>
